...
#pragma mark -
@implementation XP_PTChannel

@synthesize protocol = protocol_;


+ (XP_PTChannel*)channelWithDelegate:(id<XP_PTChannelDelegate>)delegate {
  return [[XP_PTChannel alloc] initWithProtocol:[XP_PTProtocol sharedProtocolForQueue:dispatch_get_main_queue()] delegate:delegate];
}
...
- (id)initWithProtocol:(XP_PTProtocol*)protocol {
  if (!(self = [super init])) return nil;
  protocol_ = protocol;
...
- (id)init {
  ...
  return [self initWithProtocol:[XP_PTProtocol sharedProtocolForQueue:dispatch_get_main_queue()]];
}
...
- (id<XP_PTChannelDelegate>)delegate {
  return delegate_;
}


- (void)setDelegate:(id<XP_PTChannelDelegate>)delegate {
  delegate_ = delegate;
  delegateFlags_ = 0;
  if (!delegate_) {
    return;
...
- (void)connectToPort:(int)port overUSBHub:(XP_PTUSBHub*)usbHub deviceID:(NSNumber*)deviceID callback:(void(^)(NSError *error))callback {
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError *error, XP_PTAddress *address))callback {
  assert(protocol_ != NULL);
  if (connState_ != kConnStateNone) {
  ...
  // Success
  NSError *err = nil;
  XP_PTAddress *ptAddr = [[XP_PTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)acceptIncomingConnection:(dispatch_fd_t)serverSocketFD {
  ...
  if (delegateFlags_ & kDelegateFlagImplements_ioFrameChannel_didAcceptConnection_fromAddress) {
    XP_PTChannel *peerChannel = [[XP_PTChannel alloc] initWithProtocol:protocol_ delegate:delegate_];
    __block XP_PTChannel *localChannelRef = self;
    ...
    XP_PTAddress *address = [[XP_PTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
        ...
        // Read and discard payload, ignoring frame
          ...
          if (handleError(error, bufferSize == 0)) {
            return;
              ...
              if (self->delegate_) {
            XP_PTData *payload = [[XP_PTData alloc] initWithMappedDispatchData:contiguousData data:(void*)buffer length:bufferSize];
...
- (NSString*)description {
  ...
  return [NSString stringWithFormat:@"<XP_PTChannel: %p (%@)%s%@>", self, (  connState_ == kConnStateConnecting ? @"connecting"
...
- (NSString*)description {
  return [NSString stringWithFormat:@"<XP_PTData: %p (%zu bytes)>", self, length_];
}

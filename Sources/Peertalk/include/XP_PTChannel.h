...
@class XP_PTData, XP_PTAddress;
@protocol XP_PTChannelDelegate;

@interface XP_PTChannel : NSObject

// Delegate
@property (strong) id<XP_PTChannelDelegate> delegate;

// Communication protocol. Must not be nil.
@property XP_PTProtocol *protocol;
...
// Create a new channel using the shared XP_PTProtocol for the current dispatch
// queue, with *delegate*.
...
// protocol instance (as returned by [XP_PTProtocol sharedProtocolForQueue:
...
@interface XP_PTAddress : NSObject
...
// Protocol for XP_PTChannel delegates
@protocol XP_PTChannelDelegate <NSObject>

@required
// Invoked when a new frame has arrived on a channel.
- (void)ioFrameChannel:(XP_PTChannel*)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(XP_PTData*)payload;

@optional
// Invoked to accept an incoming frame on a channel. Reply NO ignore the
// incoming frame. If not implemented by the delegate, all frames are accepted.
- (BOOL)ioFrameChannel:(XP_PTChannel*)channel shouldAcceptFrameOfType:(uint32_t)type tag:(uint32_t)tag payloadSize:(uint32_t)payloadSize;

// Invoked when the channel closed. If it closed because of an error, *error* is
// a non-nil NSError object.
- (void)ioFrameChannel:(XP_PTChannel*)channel didEndWithError:(NSError*)error;

// For listening channels, this method is invoked when a new connection has been
// accepted.
- (void)ioFrameChannel:(XP_PTChannel*)channel didAcceptConnection:(XP_PTChannel*)otherChannel fromAddress:(XP_PTAddress*)address;

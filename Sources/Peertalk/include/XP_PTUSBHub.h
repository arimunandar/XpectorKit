...
// XP_PTUSBDeviceDidAttachNotification
// Posted when a device has been attached. Also posted for each device that is
// already attached when the XP_PTUSBHub starts listening.
//
//  .userInfo = {
//    DeviceID = 3;
...
//  }
//
FOUNDATION_EXPORT NSString * const XP_PTUSBDeviceDidAttachNotification;

// XP_PTUSBDeviceDidDetachNotification
...
//  }
//
FOUNDATION_EXPORT NSString * const XP_PTUSBDeviceDidDetachNotification;

// NSError domain
FOUNDATION_EXPORT NSString * const XP_PTUSBHubErrorDomain;

// Error codes returned with NSError.code for NSError domain XP_PTUSBHubErrorDomain
typedef enum {
  PTUSBHubErrorBadDevice = 2,
  PTUSBHubErrorConnectionRefused = 3,
} PTUSBHubError;

@interface XP_PTUSBHub : NSObject

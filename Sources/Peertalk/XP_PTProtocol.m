...
NSString * const XP_PTProtocolErrorDomain = @"PTProtocolError";
...
static void _release_queue_local_protocol(void *objcobj) {
  if (objcobj) {
    XP_PTProtocol *protocol = (__bridge_transfer id)objcobj;
    protocol->queue_ = NULL;
  }
}


@interface XP_RQueueLocalIOFrameProtocol : XP_PTProtocol
@end
@implementation XP_RQueueLocalIOFrameProtocol
- (void)setQueue:(dispatch_queue_t)queue {
}
@end


@implementation XP_PTProtocol


+ (XP_PTProtocol*)sharedProtocolForQueue:(dispatch_queue_t)queue {
  static const char currentQueueFrameProtocolKey;
  //dispatch_queue_t queue = dispatch_get_current_queue();
  XP_PTProtocol *currentQueueFrameProtocol = (__bridge XP_PTProtocol*)dispatch_queue_get_specific(queue, &currentQueueFrameProtocolKey);
  if (!currentQueueFrameProtocol) {
    currentQueueFrameProtocol = [[XP_RQueueLocalIOFrameProtocol alloc] initWithDispatchQueue:NULL];
    ...
    return (__bridge XP_PTProtocol*)dispatch_queue_get_specific(queue, &currentQueueFrameProtocolKey); // to avoid race conds
  } else {
...
#endif
        callback([[NSError alloc] initWithDomain:XP_PTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
        return;
      }
...
@interface XP_PTDispatchData : NSObject {
  dispatch_data_t dispatchData_;
}
@end
@implementation XP_PTDispatchData
...
@implementation NSData (XP_PTProtocol)
...
+ (NSData *)dataWithContentsOfDispatchData:(dispatch_data_t)data {
  ...
  XP_PTDispatchData *dispatchDataRef = [[XP_PTDispatchData alloc] initWithDispatchData:contiguousData];
...
@implementation NSDictionary (XP_PTProtocol)

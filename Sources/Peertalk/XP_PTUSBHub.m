...
NSString * const XP_PTUSBHubErrorDomain = @"PTUSBHubError";
...
NSString * const XP_PTUSBDeviceDidAttachNotification = @"XP_PTUSBDeviceDidAttachNotification";
NSString * const XP_PTUSBDeviceDidDetachNotification = @"XP_PTUSBDeviceDidDetachNotification";
...
// (device) system. In practice, a XP_PTUSBChannel is connected to a usbmuxd
...
// let XP_PTUSBHub provide you with already configured channels.
@interface XP_PTUSBChannel : NSObject {
...
// own I/O logic without XP_PTUSBChannel. Remember to dispatch_retain() the channel
// if you plan on using it as it might be released from the XP_PTUSBChannel at any
// point in time.
...
@interface XP_PTUSBChannel (Private)
...
@interface XP_PTUSBHub () {
  XP_PTUSBChannel *channel_;
}
- (void)handleBroadcastPacket:(NSDictionary*)packet;
@end


@implementation XP_PTUSBHub


+ (XP_PTUSBHub*)sharedHub {
  static XP_PTUSBHub *gSharedHub;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    gSharedHub = [XP_PTUSBHub new];
    [gSharedHub listenOnQueue:dispatch_get_main_queue() onStart:^(NSError *error) {
      if (error) {
        NSLog(@"XP_PTUSBHub failed to initialize: %@", error);
      }
    } onEnd:nil];
  });
  return gSharedHub;
...
- (void)connectToDevice:(NSNumber*)deviceID port:(int)port onStart:(void(^)(NSError*, dispatch_io_t))onStart onEnd:(void(^)(NSError*))onEnd {
  XP_PTUSBChannel *channel = [XP_PTUSBChannel new];
  NSError *error = nil;
  ...
  NSDictionary *packet = [XP_PTUSBChannel packetDictionaryWithPacketType:kPlistPacketTypeConnect
...
- (void)handleBroadcastPacket:(NSDictionary*)packet {
  ...
  if ([@"Attached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:XP_PTUSBDeviceDidAttachNotification object:self userInfo:packet];
  } else if ([@"Detached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:XP_PTUSBDeviceDidDetachNotification object:self userInfo:packet];
  } else {
...
@implementation XP_PTUSBChannel
...
- (void)listenWithBroadcastHandler:(void(^)(NSDictionary *packet))broadcastHandler callback:(void(^)(NSError*))callback {
  ...
  NSDictionary *packet = [XP_PTUSBChannel packetDictionaryWithPacketType:kPlistPacketTypeListen payload:nil];
...
- (BOOL)errorFromPlistResponse:(NSDictionary*)packet error:(NSError**)error {
  if (!*error) {
    ...
    if (!n) {
      *error = [NSError errorWithDomain:XP_PTUSBHubErrorDomain code:(n ? n.integerValue : 0) userInfo:nil];
      return NO;
    }
    ...
    if (replyCode != 0) {
      ...
      *error = [NSError errorWithDomain:XP_PTUSBHubErrorDomain code:replyCode userInfo:[NSDictionary dictionaryWithObject:errmessage forKey:NSLocalizedDescriptionKey]];
      return NO;
    }
  }
  return YES;
  ...
  dispatch_io_read(channel_, 0, sizeof(ref_upacket.size), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    off_t offset = sizeof(ref_upacket.size);
      ...
      if (upacket_len > kUsbmuxPacketMaxPayloadSize) {
        callback(
          [[NSError alloc] initWithDomain:XP_PTUSBHubErrorDomain code:1 userInfo:@{
      ...
      if (upacket->protocol != USBMuxPacketProtocolPlist) {
        callback([[NSError alloc] initWithDomain:XP_PTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package protocol" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
      ...
      if (upacket->type != USBMuxPacketTypePlistPayload) {
        callback([[NSError alloc] initWithDomain:XP_PTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package type" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
